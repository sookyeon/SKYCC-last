#112번째줄 빈칸에 이미지 파일이 오게끔 하기 ===> 이때, 유저가 업로드 눌렀을때, 빈칸에 png파일로 올려지게한다. 
import cv2
import scipy
...
def run_image_convert(image_file):
    output=caart(cv2.imread(image_file))
    # cv2.imshow('Original', output)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    cv2.imwrite("cv_"+image_file, output)